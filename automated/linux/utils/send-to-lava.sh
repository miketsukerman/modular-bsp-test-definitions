...
# send-to-lava.sh
#
# Translate a plain-text result file into LAVA signals.
# Result-file format (one entry per line):
#
...
#   lava-test-set stop
#
# Based on the upstream Linaro send-to-lava.sh from
# https://github.com/Linaro/test-definitions/blob/master/automated/utils/send-to-lava.sh
#
...
show_output() {
    local test_name="$1"
    local test_output="${RESULT_DIR}/${test_name}.log"
...
if [ -f "${RESULT_FILE}" ]; then
    ...
    while IFS= read -r line; do
        ...
        elif echo "${line}" | grep -iqE ".* (pass|fail|skip|unknown) .*"; then
        ...
        elif echo "${line}" | grep -iqE "^lava-test-set.*"; then
